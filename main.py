...
with st.sidebar:
    ...
    selected_country = st.selectbox(
        "Choissisez un pays", pays_list, index=pays_list.index("France")
    )  # Par défaut France
    ...
    data_selection_method = st.radio(
        "Méthode de sélection de la date :",
        ("Date manuelle", "Date à partir de tempêtes de références"),
    )
    ...
    if data_selection_method == "Date manuelle":
        selected_date = st.date_input(
            ...
            min_value=datetime(1979, 1, 1),
            max_value=datetime.today() - timedelta(days=5),
        )
    ...
    # Selection de la date à partir du fichier tempête
        ...
        info_selected_tempete = data_tempetes[
            data_tempetes["Nom de la Tempête"] == selected_tempete
        ].iloc[0]
        ...
        if country_selected != country_selected_tempete:
            st.warning(
                " Attention : Le pays choisi ne correspond pas au pays de la tempête sélectionnée"
            )
        else:
    ...
    wind_selected = st.radio(
        "Choix de la variable de vent", ("rafale", "soutenu_10m", "soutenu_100m")
    )
    ...
    if resolution_parent > resolution_base:
        ...
        if resolution_parent <= 8:
            area_km2 = round(
                h3.cell_area(
                    h3.latlng_to_cell(48.8566, 2.3522, resolution_parent), unit="km^2"
                )
            )  # surface d'un hexagone sur un pt aléatoire (Paris)
            st.success(
                f"Vous avez sélectionné la résolution {resolution_parent} d'une surface de {area_km2} km²"
            )
        else:
            area_km2 = round(
                h3.cell_area(
                    h3.latlng_to_cell(48.8566, 2.3522, resolution_parent), unit="m^2"
                )
            )  # surface d'un hexagone sur un pt aléatoire (Paris)
            st.success(
...
name_folder = (
    "wind_api_copernicus"  # Sous dossier dans lequel les fichiers .nc sont enregistrés
)
    ...
    if os.path.exists(os.path.join(name_folder, filename)):
        ...
        with carte_side:
            ...
            if "dataset" not in st.session_state:
                ...
                longitudes, latitudes, wind_mag = traitement_data_wind(
                    dataset, wind_selected
                )
            ...
            if "hexagones" not in st.session_state:
                ...
                hexagones = calcul_hexagone(
                    latitudes, longitudes, wind_mag, resolution_base, resolution_parent
                )
            ...
            if "carte_with_hexagons" not in st.session_state:
                ...
                st.session_state.carte = affichage_hexagones(
                    st.session_state.carte, hexagones, wind_selected
                )
        ...
        st.warning("Pour afficher la carte cliquez sur commencer le téléchargement")
